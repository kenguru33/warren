...
- **Discovery is multicast and Docker's bridge blocks it**, so both stacks fall back to a throttled unicast scan of the `WARREN_LAN_IP` subnet — Sonos on `:1400`, Cast on `:8008/setup/eureka_info`. Without that, a containerized deployment discovers nothing at all. See [`docker/CLAUDE.md`](docker/CLAUDE.md).
