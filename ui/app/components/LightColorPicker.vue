<script setup lang="ts">
const props = defineProps<{ modelValue: string | null }>()
const emit = defineEmits<{ (e: 'update:modelValue', hex: string): void }>()

const PRESETS: { name: string; hex: string }[] = [
  { name: 'Warm white', hex: '#fff6dd' },
  { name: 'Cool white', hex: '#e0eaff' },
  { name: 'Amber',      hex: '#ffaa33' },
  { name: 'Red',        hex: '#ff2d2d' },
  { name: 'Orange',     hex: '#ff7a00' },
  { name: 'Yellow',     hex: '#ffd400' },
  { name: 'Green',      hex: '#3fcf3f' },
  { name: 'Teal',       hex: '#00d1b2' },
  { name: 'Blue',       hex: '#1e90ff' },
  { name: 'Indigo',     hex: '#6366f1' },
  { name: 'Purple',     hex: '#a855f7' },
  { name: 'Pink',       hex: '#ec4899' },
]

function pick(hex: string) {
  emit('update:modelValue', hex)
}

function onCustomInput(e: Event) {
  const v = (e.target as HTMLInputElement).value
  if (v) emit('update:modelValue', v)
}
</script>

<template>
  <div class="grid grid-cols-6 gap-2 sm:grid-cols-7">
    <button
      v-for="c in PRESETS"
      :key="c.hex"
      type="button"
      :class="[
        'size-9 rounded-full transition hover:scale-110 focus:outline-none focus-visible:ring-2 focus-visible:ring-accent ring-1',
        modelValue?.toLowerCase() === c.hex.toLowerCase()
          ? 'ring-2 ring-accent ring-offset-2 ring-offset-modal'
          : 'ring-default dark:ring-white/15',
      ]"
      :style="{ background: c.hex }"
      :title="c.name"
      :aria-label="c.name"
      @click="pick(c.hex)"
    />
    <label
      class="relative size-9 rounded-full overflow-hidden ring-1 ring-default cursor-pointer dark:ring-white/15"
      title="Custom color"
    >
      <input
        type="color"
        class="absolute inset-0 size-full cursor-pointer opacity-0"
        :value="modelValue ?? '#ffffff'"
        @input="onCustomInput"
      />
      <span class="absolute inset-0 bg-[conic-gradient(from_0deg,_#ef4444,_#f59e0b,_#eab308,_#22c55e,_#06b6d4,_#3b82f6,_#a855f7,_#ec4899,_#ef4444)]" />
    </label>
  </div>
</template>
