...
- **Play means resume, not "start a favorite".** A stopped Sonos speaker is not an empty one — it usually still holds its queue, or a station started from the Sonos app. Pressing play continues that; loading a favorite happens only when the user picks one. Verified against hardware: YouTube Music content queued from the YouTube Music app (`x-sonosapi-hls-static:…sid=284`) is fully readable and controllable from the tile — title, artist, artwork, transport and volume — which is the remote-control half of this feature working for exactly the content it was built for.
